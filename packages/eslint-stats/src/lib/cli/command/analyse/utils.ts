...
import {groupByOptions, InteractiveCommandState, maxGroupByLength, maxSortByLength, sortByOptions} from "./command-state";

export function createInteractiveOptions(state: InteractiveCommandState): string {
    const {
        groupByIndex,
        sortByIndex,
        sortOrder,
